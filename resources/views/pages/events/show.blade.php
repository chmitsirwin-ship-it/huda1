        ...
        <div class="flex flex-wrap gap-3 mb-8">
            ...
            <span class="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 rounded-full px-3 py-1.5 text-sm font-medium">
                ...
                <span>
                    ...
                    <span class="block text-[10px] text-emerald-500 font-normal">{{ \App\Support\LocalizedDate::hijri($event->starts_at) }}</span>
                </span>
            </span>
            ...
            @if($event->starts_at)
                <span class="inline-flex items-center gap-2 bg-neutral-100 text-neutral-600 rounded-full px-3 py-1.5 text-sm font-medium">
                    ...
                    {{ \App\Support\LocalizedDate::time($event->starts_at) }}
                    @if($event->ends_at)
                        &mdash; {{ \App\Support\LocalizedDate::time($event->ends_at) }}
                    @endif
                </span>
            @endif
