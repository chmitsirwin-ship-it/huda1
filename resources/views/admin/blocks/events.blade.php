<div class="rounded-2xl border border-emerald-200 overflow-hidden shadow-sm bg-white">
    <div class="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-emerald-800 to-emerald-700 relative overflow-hidden">
        ...
        <x-icon name="heroicon-o-calendar-days" class="relative size-5 text-amber-300 shrink-0"/>
        <span class="relative text-sm font-semibold text-white tracking-wide">{{ __('Events') }}</span>
        <span class="relative ms-auto text-xs text-emerald-300 font-medium">الفعاليات</span>
    </div>
    <div class="p-4 bg-stone-50 space-y-3">
        <div class="grid grid-cols-3 gap-2">
            @foreach(range(1, 3) as $i)
                <div class="border border-emerald-100 rounded-xl p-2 bg-white">
                    <div class="h-8 rounded bg-emerald-50 mb-2 flex items-center justify-center">
                        <x-icon name="heroicon-o-calendar-days" class="size-4 text-emerald-400"/>
                    </div>
                    <div class="h-2 bg-stone-200 rounded w-full mb-1"></div>
                    <div class="h-2 bg-stone-100 rounded w-3/4"></div>
                </div>
            @endforeach
        </div>
